...
float FuelInjectionControl::airDensity(float MAT, float MAP) {
	return (1.2929 * 273.13 / (MAT + 273.13) * MAP / 101.325);
}
...
//    Array::assign(this->OxygenLoopTable, z, 2, EGO_ROWS, 0, EGO_COLS);
	...
	return (warmUp / 100) * (O2ClosedLoop / 100);
...
//    Array::assign(this->VETable, z, 2, VE_ROWS, 0, VE_COLS);
	...
	return requiredFuel(MAT, MAP, RPM) * VE * (MAP / 100.0) * gammaEnrichment(CLT, EGO, MAP, RPM) + INJ_OPEN_TIME;
}
