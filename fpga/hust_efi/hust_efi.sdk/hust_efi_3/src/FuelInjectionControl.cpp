...
//  Array::assign(this->VETable, z, 2, VE_ROWS, 0, VE_COLS);
    for (int i = 2; i < VE_ROWS; ++i)
    {
    	...
    	{
    		z[i-2][j] = this->VETable[i][j];
    	}
    }

    float VE = Math::interpolation2d(x, y, z, RPM, MAP, VE_ROWS - 2, VE_COLS);

    float gamma = gammaEnrichment(CLT, EGO, MAP, RPM);

    cout << "\nVE o day: " << VE << endl;

    cout << "gamma: " << gamma << endl;

	return requiredFuel(MAT, MAP, RPM) * (VE / 100.0) * (MAP / 100.0) * gamma + INJ_OPEN_TIME;
}
