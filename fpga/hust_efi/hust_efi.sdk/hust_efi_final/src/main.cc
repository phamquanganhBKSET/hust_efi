...
void vTaskProcess(void* parameter) {
	...
	for(;;) {
		...
		if ((cal_injection) & 0x01) {
			float inj = efi->FIC[0].pulseWidth(efi->CLTSensor.getData(), efi->EGOSensor.getData(), efi->MATSensor.getData(), efi->MAPSensor.getData(), efi->getRPM());
			cout << "\nCalculate injection time for cylinder 0: inj_time = " << inj;
			cout << "\nInjecting..." << endl;
			efi->FIC[0].inject();
			cout << "\nDone inject" << endl;
			done[0] = 1;
